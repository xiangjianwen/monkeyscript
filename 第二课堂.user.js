...
my.innerHTML =
	'<div  style="position:fixed;top:20px;left:0%" lign="left" id="bg">移动<div id ="x"></div>  <textarea name="cardid1" id="cardid1" cols="4" rows="5"></textarea> <textarea name="searchId1" id="searchId1" cols="30" rows="8"></textarea><input  style="width:60px;height:100px;"  type="submit" name="tijiao" id="tijiao" value="提交" /></div>';
my.id = "divCell";
$(my).insertAfter("body")
	 var s = "";
	 var k = 0;
...
GM_xmlhttpRequest(
					 {
  method: "GET", url: "http://xiangjianwen.host3v.vip", onload:function
					 (response)
					 {
...
function readWorkbookFromRemoteFile(url, callback)
{
	...
	GM_xmlhttpRequest(
						 {
	  method: "GET", url: url, responseType: 'arraybuffer', onload:function
						 (e)
						 {
						 if (e.status == 200)
						 {
						 var data = new Uint8Array(e.response); var workbook = XLSX.read(data,
																						 {
			  type:																	 'array'}
...
function outputWorkbook(workbook)
{
	...
	sheetNames.forEach(name = >
					   ...
					   {
					   // v是读取单元格的原始值 
					   // console.log(key, key[0] === '!' ? worksheet[key] :
					   // worksheet[key].v); 
					   }
					   }
...
var wb;
// var url =
// "http://www.gjxq.gov.cn/uploadfile/2018/1008/20181008073536628.xlsx" ; 
...
$(document).ready(function()
				  ...
				  jdtecont = jdtecont.replace(/\s + /g, "");
				  jdtecont =
				  jdtecont.replace(/"([^"] *) "/g ," � ������������$1 � ������������");
     var indextk=tk.indexOf(jdtecont.substr(0,8),0);//题库查找位置 
     var qda=tk.substr(indextk,212);
     
     var ABC=qda.substr(qda.indexOf(" � ������������案",0)+3,1); 
     console.log($(".index__kind - 1L O6L "));  
     if(flag==0 && $(".index__kind - 1L O6L ").length!=0){   
       if($(".index__kind - 1L O6L ")[0].innerHTML==" � ������������选"){   
         if(ABC==" A ")  
         {var a = $(".index__radio - 3 rN1i ")[0]; $(a).trigger(" click ");  
         }   
         else if(ABC==" B "){  
           var a = $(".index__radio - 3 rN1i ")[1]; $(a).trigger(" click ");   
         }   
         else if(ABC==" C "){   
           var a = $(".index__radio - 3 rN1i ")[2]; $(a).trigger(" click ");   
         }   
         else if(ABC==" D "){ 
           var a = $(".index__radio - 3 rN1i ")[3]; $(a).trigger(" click ");   
         }    
         flag=1;  console.log(ABC); 
       }  
       else     
           {   for(var i=0;i<$(".index__multiple - 1 ORmX ").length;i++)
            {var a = $(".index__multiple - 1 ORmX ")[i];
             $(a).trigger(" click "); 
            }
           }       
          ...
          {    
            var a = $(".index__next - 1 DGWu ")[0]; $(a).trigger(" click ");     
     
             flag=0;    
       }    
     //$(".index__kind - 1L O6L ")[0].innerHTML
     //var a = $(".index__multiple - 1 ORmX ")[1]; $(a).trigger(" click ");
     //var a = $(".index__radio - 3 rN1i ")[1]; $(a).trigger(" click ");
     //var a = $(".index__next - 1 DGWu ")[0]; $(a).trigger(" click ");
     $("
#searchId1").val(qda);
